...
Config load_config_yaml(const std::string& path) { 
    ...
    if (root["gpu"]) {
        auto g = root["gpu"];
        if (g["device_id"]) cfg.device_id = c["device_id"];
    ...
    if (root["xtx"]) {
        auto x = root["xtx"];
        if (x["algorithm"]) cfg.algorithm = x["algorithm"];
        if (x["triangle"]) cfg.triangle = x["triangle"];
        if (x["alpha"]) cfg.alpha = x["alpha"];
        if (x["beta_first"]) cfg.beta_first = x["beta_first"];
        if (x["beta_rest"]) cfg.beta_rest = x["beta_rest"];
    }
